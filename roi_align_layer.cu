...
__global__ void ROIAlignForward(const int nthreads, const Dtype* bottom_data,
  ...
  CUDA_KERNEL_LOOP(index, nthreads) {
    ...
    bottom_rois += n * 5;
    Dtype roi_batch_ind = bottom_rois[0];
    ...
    Dtype roi_width = max(roi_end_w - roi_start_w + 1, 1);
    Dtype roi_height = max(roi_end_h - roi_start_h + 1, 1);
    Dtype bin_size_h = static_cast<Dtype>(roi_height)
                       / static_cast<Dtype>(pooled_height);
    Dtype bin_size_w = static_cast<Dtype>(roi_width)
                       / static_cast<Dtype>(pooled_width);

    Dtype hstart = static_cast<Dtype>(floor(static_cast<Dtype>(ph)
                                        * bin_size_h));
    Dtype wstart = static_cast<Dtype>(floor(static_cast<Dtype>(pw)
                                        * bin_size_w));
    Dtype hend = static_cast<Dtype>(ceil(static_cast<Dtype>(ph + 1)
                                     * bin_size_h));
    Dtype wend = static_cast<Dtype>(ceil(static_cast<Dtype>(pw + 1)
                                     * bin_size_w));
...
void ROIAlignLayer<Dtype>::Forward_gpu(const vector<Blob<Dtype>*>& bottom,
  ...
  Dtype* argmax_data_x = max_idx_.mutable_gpu_data();
  Dtype* argmax_data_y = max_idx_.mutable_gpu_data();
...
void ROIAlignLayer<Dtype>::Backward_gpu(const vector<Blob<Dtype>*>& top,
  ...
  const Dtype* argmax_data_x = max_idx_.gpu_data();
  const Dtype* argmax_data_y = max_idx_.gpu_data();
  // NOLINT_NEXT_LINE(whitespace/operators)
